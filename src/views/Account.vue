<template>
  <div class="Acc">
    <div class="top_box">
      <div class="total_assets">
        <p>总资产(元)</p>
        <p>{{ datas.now_money }}</p>
      </div>
      <div class="pay">充值</div>
      <div class="add_up">
        <div class="pag_consume">
          <p>累计充值(元)</p>
          <p>{{ datas.recharge }}</p>
        </div>
        <div class="pag_consume">
          <p>累计消费(元)</p>
          <p>{{ datas.orderStatusSum }}</p>
        </div>
      </div>
    </div>
    <van-grid :border="false" :column-num="4">
      <van-grid-item @click="gobill(1)">
        <van-image
          src="data:image/png;base64,"
        />
        <p>账单记录</p>
      </van-grid-item>
      <van-grid-item @click="gobill(2)">
        <van-image
          src="data:image/png;base64,"
        />
        <p>消费记录</p>
      </van-grid-item>
      <van-grid-item @click="gobill(3)">
        <van-image
          src="data:image/png;base64,"
        />
        <p>充值记录</p>
      </van-grid-item>
      <van-grid-item @click="gointe">
        <van-image
          src="data:image/png;base64,"
        />
        <p>积分中心</p>
      </van-grid-item>
    </van-grid>
    <div class="con_box">
      <div class="box_left">
        <p>签到领积分</p>
        <p>赚积分抵现金</p>
        <img src="http://47.115.51.185/h5/img/gift.bde2a03c.png" alt="" />
      </div>
      <router-link to="/user/user_coupon" tag="div" class="box_right">
        <p>领取优惠券</p>
        <p>满减享优惠</p>
        <img src="http://47.115.51.185/h5/img/money.136360a9.png" alt="" />
      </router-link>
    </div>
    <div class="bon_box">
      <div class="box">
        <img src="../assets/img/拼团.png" alt="" />
        <div class="text">
          <p>最新砍价拼团活动</p>
          <p>最新的优惠商品上架拼团</p>
        </div>
        <div :class="{ btn: true, true: !is.is_pink }">
          {{ is.is_pink ? "马上开始" : "已结束" }}
        </div>
      </div>
      <div class="box">
        <img src="../assets/img/砍价.png" alt="" />
        <div class="text">
          <p>砍价活动</p>
          <p>呼朋唤友来砍价</p>
        </div>
        <div :class="{ btn: true, true: !is.is_bargin }">
          {{ is.is_bargin ? "马上开始" : "已结束" }}
        </div>
      </div>
      <div class="box">
        <img src="../assets/img/秒杀.png" alt="" />
        <div class="text">
          <p>当前现时秒杀</p>
          <p>最新商品秒杀进行中</p>
        </div>
        <div :class="{ btn: true, true: !is.is_seckill }">
          {{ is.is_seckill ? "马上开始" : "已结束" }}
        </div>
      </div>
    </div>
    <div class="bon_hot">
      <hot></hot>
    </div>
  </div>
</template>

<script>
import Hot from "../components/Hot.vue";
export default {
  data() {
    return {
      datas: [],
      is: [],
    };
  },
  components: {
    Hot,
  },
  methods: {
    gobill(e) {
      console.log("/bill/");
      this.$router.push(`/bill/${e}`);
    },
    gointe() {
      this.$router.push(`/integral`);
    },
  },
  created() {
    this.axios.get("/api/user/balance").then((d) => {
      this.datas = d.data.data;
    });
    this.axios.get("/api/user/activity").then((d) => {
      console.log(d);
      this.is = d.data.data;
    });
  },
};
</script>

<style lang="less" scoped>
.true {
  color: rgb(137, 137, 137) !important;
  background-color: rgb(236, 236, 236) !important;
}
.Acc {
  padding: 15px;
  .bon_hot {
    margin-top: 30px;
  }
  .top_box {
    height: 150px;
    width: 100%;
    background-image: linear-gradient(90deg, #f33b2b 0, #f36053);
    border-radius: 10px;
    position: relative;
    .total_assets {
      padding: 15px;
      color: white;
      p {
        &:first-child {
          color: rgba(255, 255, 255, 0.7);
          font-size: 12px;
        }
        &:last-child {
          margin-top: 10px;
          font-size: 32px;
        }
      }
    }
    .add_up {
      padding: 0 15px;
      color: white;
      display: flex;
      .pag_consume {
        flex: 1;
        p {
          &:first-child {
            color: rgba(255, 255, 255, 0.7);
            font-size: 12px;
          }
          &:last-child {
            margin-top: 10px;
            font-size: 16px;
          }
        }
      }
    }
    .pay {
      position: absolute;
      top: 15px;
      right: 15px;
      font-size: 12px;
      padding: 5px 25px;
      border-radius: 20px;
      color: #f33b2b;
      background-color: white;
    }
  }
  .van-grid {
    .van-grid-item {
      .van-image {
        width: 24px;
        height: 24px;
      }
      p {
        margin-top: 10px;
        font-size: 12px;
        color: rgb(181, 181, 181);
      }
    }
  }
  .con_box {
    display: flex;
    justify-content: space-between;
    .box_left,
    .box_right {
      width: 48%;
      height: 55px;
      background-color: rgb(255, 246, 209);
      padding: 10px;
      box-sizing: border-box;
      position: relative;
      p {
        &:first-child {
          font-weight: 900;
          font-size: 14px;
          color: rgb(246, 101, 79);
        }
        &:nth-child(2) {
          font-size: 12px;
          color: rgb(246, 101, 79);
        }
      }
      img {
        position: absolute;
        width: 36px;
        height: 36px;
        position: absolute;
        right: 10px;
        top: 50%;
        margin-top: -18px;
      }
    }
    .box_right {
      background-color: rgb(255, 243, 243);
    }
  }
  .bon_box {
    margin-bottom: 10px;
    .box {
      margin-top: 20px;
      display: flex;
      align-items: center;
      img {
        width: 40px;
        height: 40px;
      }
      .text {
        margin-left: 10px;
        flex: 1;
        font-size: 14px;
        p {
          &:last-child {
            color: rgb(181, 181, 181);
          }
        }
      }
      .btn {
        width: 80px;
        height: 30px;
        color: white;
        background-color: rgb(107, 179, 252);
        border-radius: 20px;
        font-size: 12px;
        text-align: center;
        line-height: 30px;
      }
    }
  }
}
</style>